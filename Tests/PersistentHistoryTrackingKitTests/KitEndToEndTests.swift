...
struct KitEndToEndTests {
  ...
  func kitAutoSyncStartStop() async throws {
    let storeURL = FileManager.default.temporaryDirectory
      .appendingPathComponent("TestModel_KitAutoSync_\(UUID().uuidString).sqlite")

    let writerContainer = TestModelBuilder.createContainer(
      author: "App1",
      testName: #function,
      storeURL: storeURL,
      resetStoreFiles: true)
    let listenerContainer = TestModelBuilder.createContainer(
      author: "App2",
      testName: #function,
      storeURL: storeURL,
      resetStoreFiles: false)

    let context1 = writerContainer.newBackgroundContext()
    let context2 = listenerContainer.newBackgroundContext()
    ...
    let timestampKey = uniqueString + "App2"
    NotificationCenter.default.post(
      name: .NSPersistentStoreRemoteChange,
      object: listenerContainer.persistentStoreCoordinator)
    let firstProcessedTimestamp = try await waitForStoredTimestamp(
      in: userDefaults,
      key: timestampKey)

    kit.stop()

    try await app1Handler.createPerson(name: "Charlie", age: 26, author: "App1")
    NotificationCenter.default.post(
      name: .NSPersistentStoreRemoteChange,
      object: listenerContainer.persistentStoreCoordinator)
    try await Task.sleep(nanoseconds: 200_000_000)

    let stoppedTimestamp = userDefaults.object(forKey: timestampKey) as? Date
    #expect(stoppedTimestamp == firstProcessedTimestamp)
  }
...
private func waitForStoredTimestamp(
  ...
  Issue.record("Timed out waiting for timestamp at key \(key)")
  return try #require(userDefaults.object(forKey: key) as? Date)
}
